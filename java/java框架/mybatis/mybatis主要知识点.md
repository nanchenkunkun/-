...
```java
    <insert id="savaBatch"  parameterType="java.util.List">
        INSERT INTO LETTER_CUSTINFO(
            ID,
            CUST_ID
        )
        SELECT SEQ_LETTER_CUSTINFO.NEXTVAL,t.*
        FROM (
...
<Select id="selectuser" paramterType="User">
...
B.在需要开启二级缓存的mapper.xml中加入cache标签
